...
from DAXXMUSIC.utils.ai_dj import PlayMessageProxy, parse_music_request
...
async def natural_language_play(client, message: Message):
    query = parse_music_request(message.text)
    if not query:
        return
    await play_commnd(client, PlayMessageProxy(message, query))
